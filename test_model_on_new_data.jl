...
include("preprocess_idx.jl")
include("validation_dataset.jl")
include("minibatch.jl")
include("model.jl")

# Load model and datasets onto GPU, if enabled
train_set2 = gpu.(train_set2)
validation_set2 = gpu.(validation_set2)
model = gpu(model)

# Make sure our model is nicely precompiled before starting our training loop
model(train_set2[1][1])
model(train_set2[1][1])[:, :, 1, 32] #see last output
...
run = @time @elapsed for epoch_idx in 1:5
    ...
    acc = mean([accuracy(x, y) for (x, y) in validation_set2]) #separating validation set tuple into the input and outputs & checking the accuracy between x and y; then getting mean
    ...
    if acc >= best_acc
        ...
        BSON.@save joinpath(dirname(@__FILE__), "BSON/r2_$(Stride)x$(Stride).bson") params=cpu.(params(model)) epoch_idx acc
        best_acc = acc
