...
run = @time @elapsed for epoch_idx in 1:200
    ...
    Flux.train!(loss, params(model), train_set_slide, opt)
    ...
    acc = mean([accuracy(x, y) for (x, y) in validation_set_slide]) #separating validation set tuple into the input and outputs & checking the accuracy between x and y; then getting mean
